...
def run_kmeans(input_path, output_dir="data/kmeans", n_clusters=3):
    ...
    plt.grid(True)
    plt.savefig(f"{output_dir}/Elbow_Plot.png", format='png', bbox_inches='tight')
    ...
    if len(rfm['Cluster'].unique()) > 1 and len(rfm_val) > n_clusters:
        silhouette_avg = silhouette_score(rfm_scaled, rfm['Cluster'])
        print(f"Silhouette Score: {silhouette_avg:.4f}")
    else:
        print("Không đủ dữ liệu hoặc cụm để tính Silhouette Score.")

    # 2. Tính WCSS (Within-Cluster Sum of Squares)
    wcss = kmeans_scaled.inertia_
    ...
    # Lưu kết quả RFM vào file CSV để tra cứu
    rfm.to_csv(f"{output_dir}/rfm_results.csv", index=False)
